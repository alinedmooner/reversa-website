...
export default function DecryptedText({
  ...
  useEffect(() => {
    ...
    const observerCallback = (entries: IntersectionObserverEntry[]) => {
      entries.forEach(entry => {
        if (entry.isIntersecting && !hasAnimated && !reducedMotion) {
          triggerDecrypt();
          setHasAnimated(true);
        }
      });
  ...
  }, [animateOn, hasAnimated, reducedMotion, triggerDecrypt]);
  ...
  return (
    <LazyMotion features={domAnimation}>
      <m.span className={parentClassName} ref={containerRef} style={styles.wrapper} {...animateProps} {...props}>
        {/* Always the real text — the scrambled version is visual-only. */}
